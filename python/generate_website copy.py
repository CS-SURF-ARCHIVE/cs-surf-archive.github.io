...
def create_collapsible(data):
    ...
    for row in data[1:]:  # Skip the header row
        ...
        for index, item in enumerate(row[0:], start=0): 
            ...
            if index == map_name_index:
            ...
            elif index == overflow_index and item is "yes":
                overflow = True
                ...
                site_link = f'<a href="{drive_link_from_sheet}">{map_name}.zip</a>' # don't feel like calling drive api every time to get file type.  it's always zip for downloads
